...
class _DropDownTextFieldState extends State<DropDownTextField> {
  ...
  Widget build(BuildContext context) {
    return DropdownButtonFormField2(
      ...
      isExpanded: true, style: Theme.of(context).textTheme.bodySmall!.copyWith(color: stepsColor, fontSize: 14),
      hint: Text(widget.title ?? "", style: Theme.of(context).textTheme.bodySmall!.copyWith(color: stepsColor, fontSize: 14)),
      items: widget.items,
      onChanged: widget.onChanged,
      buttonStyleData: ButtonStyleData(height: 50, padding: const EdgeInsets.only(left: 0, right: 10),
          decoration: BoxDecoration(color: Colors.transparent, borderRadius: BorderRadius.circular(5), border: Border.all(color: fagoSecondaryColor, width: 0.7))),

                                  ...
                                  child: GestureDetector(
                                    onTap: ()async{
                                      if(_paymentController.amount.text.isEmpty){
                                        ...
                                        progress!.show();
                                        _paymentController.generatePaymentPin().then((value) {
                                         // print(value.body);
                                          var data = jsonDecode(value.body);
                                          if(value.statusCode == 200){
                                            print( data['data']['link']);
                                              localStorage.setString('paymentLink', data['data']['link']).then((value) {
                                                Navigator.pop(context,data['data']['link']);
                                              });
                                          }else{
                                            Get.snackbar('Error', '${data['data']['error']}');
                                          }
                                          progress.dismiss();
                                        });
                                      }
